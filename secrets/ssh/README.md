...
OpenBao can be used to [sign SSH certificates](https://openbao.org/docs/secrets/ssh/signed-ssh-certificates/) for both servers and clients.
...
1.  The example can be started as shown below.
    ...
    The keys used to configure OpenBao's CA have already been generated and the public key added to the OpenSSH server's TrustedUserCAKeys configuration.
...
1.  Ask OpenBao to sign your public key, saving the result to a file.
    ```bash
...
1. Now the signed certificate along with the corresponding private key can be used to SSH to the OpenSSH server.
    ```bash
